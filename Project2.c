...
int main(int argc, char** argv)
{
	...
	/* Do First In First Out Algorithm */
	...
	else if(strcmp(argv[1], "priority") == 0)
	{
		...
		/* Modify Task 4 priority */
		if(atoi(argv[2]) == 4)
		{
			/* Lower Task 4 priority */
			if(strcmp(argv[3], "low") == 0)
			{
...
void* task2(void* param)
{
	...
	int maxindex = 200000;
	double datapoints[maxindex]; /* array to hold all numbers in file */
	...
	if(d)
	{
		while((dir = readdir(d)) != NULL)
		{
			...
			if(file[strlen(file)-1] == 'v')
			{
				...
				int rows = 0;
				int columns = 0;
				int nonzero = 0; /* Count number of nonzero elements */
				char *ep;
				...
				while(fgets(line, 1024, stream))
				{
					...
					for(tok = strtok(tmp, ","); tok && &tok; tok = strtok(NULL, ",\n"))
					{
						...
						/* If the element is nonzero numeral */
						else if(num != 0)
						{
							nonzero++;
						}
						/* If the element begins with 0 */
						else if(tok[1] == '.')
						{
							int j = 0;
							int notzero = 0;
